...
const users = [{
  ...
  tokens: [{
    access: 'auth',
    token: jwt.sign({_id: userOneId, access: 'auth'}, 'abc123').toString() // abc123 is the secret
  }]
}, {
  _id: userTwoId,
  ...
  tokens: [{
    access: 'auth',
    token: jwt.sign({_id: userTwoId, access: 'auth'}, 'abc123').toString() // abc123 is the secret
  }]
}];
